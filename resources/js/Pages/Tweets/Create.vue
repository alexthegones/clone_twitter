    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form @submit.prevent="postTweet">
                <textarea ref="tweetTextarea" @input="resizeTweetTextarea" placeholder="Que se passe-t-il ?"
                ...
                <div class="flex items-center space-x-4 justify-end mt-3">
                    <p class="text-sm text-gray-400 font-thin" :class="{ 'text-red-500' : calculateRemainingChar < 0 }">
                        {{ calculateRemainingChar }} caractères restants</p>
                    <button-vue :disabled="!canSubmit"
                                class="bg-blue-500 hover:bg-blue-800 rounded-full font-extrabold">Tweet
                    </button-vue>
                </div>
            </form>
        </div>
